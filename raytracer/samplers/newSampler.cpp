...
newSampler::newSampler(void) : num_samples(1), num_sets(83), count(0), jump(0) {
  samples.reserve(num_samples * num_sets);
  setup_shuffled_indices();
}

newSampler::newSampler(const int ns)
    : num_samples(ns), num_sets(83), count(0), jump(0) {
  samples.reserve(num_samples * num_sets);
  setup_shuffled_indices();
}

newSampler::newSampler(const int ns, const int n_sets)
    : num_samples(ns), num_sets(n_sets), count(0), jump(0) {
  samples.reserve(num_samples * num_sets);
  setup_shuffled_indices();
}

newSampler::newSampler(const newSampler& s)
...
newSampler& newSampler::operator=(const newSampler& rhs) {
...
newSampler::~newSampler(void) {}

void newSampler::set_num_sets(const int np) { num_sets = np; }

int newSampler::get_num_samples(void) { return (num_samples); }

void newSampler::shuffle_x_coordinates(void) {
...
void newSampler::shuffle_y_coordinates(void) {
  for (int p = 0; p < num_sets; p++)
    for (int i = 0; i < num_samples - 1; i++) {
      int target = rand_int() % num_samples + p * num_samples;
...
void newSampler::map_samples_to_unit_disk(void) {
...
void newSampler::map_samples_to_hemisphere(const float exp) {
  int size = samples.size();
...
void newSampler::map_samples_to_sphere(void) {
  float r1, r2;
  float x, y, z;
  float r, phi;
...
Point2D newSampler::sample_unit_square(void) {
  if (count % num_samples == 0)  // start of a new pixel
    jump = (rand_int() % num_sets) *
           num_samples;  // random index jump initialised to zero in constructor

  return (samples[jump + shuffled_indices[jump + count++ % num_samples]]);
}

Point2D newSampler::sample_unit_disk(void) {
  if (count % num_samples == 0)  // start of a new pixel
...
Point3D newSampler::sample_hemisphere(void) {
  if (count % num_samples == 0)  // start of a new pixel
    jump = (rand_int() % num_sets) * num_samples;
...
Point3D newSampler::sample_sphere(void) {
  if (count % num_samples == 0)  // start of a new pixel
    jump = (rand_int() % num_sets) * num_samples;
...
Point2D newSampler::sample_one_set(void) {
